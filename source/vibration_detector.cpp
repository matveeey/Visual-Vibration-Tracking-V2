...
{
	// если вектор пустой - просто добавл€ем новую точку
...
{
	// создаем вектор ID (номеров) дл€ точек, которые удалим
	...
	while (!point_ids_to_be_deleted_.empty())
	{
		...
		// мен€ем номера других точек в списке, если там ещЄ что-то есть
		for (int i = 0; i < point_ids_to_be_deleted_.size(); i++)
		{
			// уменьшаем номер оставшихс€ в очереди на удаление точек на один, тк номера в vec_point_handlers сместились на один вниз (влево и тп, как удобнее дл€ понимани€ :))
...
void VibrationDetector::TrackAndCalc()
{
	...
	// провер€ем равны ли размеры векторов хэндлеров точек и найденных точек на картинке
	...
	// закидываем найденные значени€ обратно в хэндлер точек
	for (int i = 0; i < vec_point_handlers_.size(); i++)
	{
...
void VibrationDetector::ExecuteVibrationDetection()
{
	...
	while ((current_num_of_frame < amount_of_frames - 1) && running_ == true)
	{
		...
		if (!vec_point_handlers_.empty())
		{
			// “рекинг и вычисление частоты вибрации
			TrackAndCalc();
